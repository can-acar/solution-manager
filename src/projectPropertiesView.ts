...
import { getWebviewUiStyles } from '#src/webviewUi';
  ...
  panel.webview.onDidReceiveMessage(
    async (message) => {
      ...
      if (message && (message.command === 'applyProperties' || message.command === 'saveAndClose')) {
        try {
          let nextText = await updateProjectProperties(project.path, message.values || {});
          ...
          if (packageReferenceOperations.length > 0) {
            nextText = await updateProjectItemReferences(project.path, packageReferenceOperations);
          }

          const referenceOperations = createReferenceOperations(message.values?.references || []);

          if (referenceOperations.length > 0) {
            nextText = await updateProjectItemReferences(project.path, referenceOperations);
          }

          const projectItemOperations = createProjectItemOperations(message.values?.projectItems || []);

          if (projectItemOperations.length > 0) {
            nextText = await updateProjectItemReferences(project.path, projectItemOperations);
          }

          await updateProjectLaunchSettings(project.path, message.values?.launchProfiles || []);
          const nextMetadata = readProjectMetadataFromText(nextText, project.path);
          nextMetadata.launchSettings = await readProjectLaunchSettings(project.path);
      ...
      if (message && message.command === 'projectAction' && onAction) {
        try {
          ...
          if (result?.refreshProperties) {
            ...
            const nextMetadata = readProjectMetadataFromText(Buffer.from(buffer).toString('utf8'), project.path);
            nextMetadata.launchSettings = await readProjectLaunchSettings(project.path);
...
function getProjectPropertiesHtml(project, metadata, nonce, notice = '') {
  ...
  const configurationNavItems = displayConfigurations.map((configuration) => navItem(
    configuration.id,
    configuration.label,
    outputType,
    countConfigurationProperties(configuration.source)
  ));
  const configurationPaneMarkup = displayConfigurations.map((configuration) => configurationPane(
    configuration,
    outputType,
    targetFramework
  )).join('');
...
function launchProfileEditor(profile, index) {
  ...
  return `<div class="section">
    ...
    ${textareaLaunchRow('Environment variables:', index, originalName, 'environmentVariables', serializeEnvironmentVariables(profile.environmentVariables || {}))}
  </div>`;
}
...
function publishProfilesTable(profiles) {
  ...
  return actionTable(
    ['Name', 'Method', 'Configuration', 'Target', 'Runtime', 'Output', 'Actions'],
    rows,
...
function centralPackageVersionsTable(versions) {
  ...
  return actionTable(
    ['Package', 'Version', 'Item Condition', 'Group Condition', 'File', 'Actions'],
    rows,
    'No central package versions found.'
  );
}
...
function nugetPackageSourcesTable(sources) {
  ...
  return actionTable(
    ['Source', 'URL/Path', 'Protocol', 'Status', 'Actions'],
    rows,
...
function nugetPackageSourceMappingsTable(mappings) {
  ...
  return actionTable(
    ['Mapped Source', 'Package Patterns', 'Actions'],
    rows,
    'No package source mappings found.'
  );
}
...
function msbuildTargetsTable(targets) {
  ...
  return actionTable(
    ['Target', 'Before', 'After', 'Depends On', 'Condition', 'Tasks', 'Actions'],
    rows,
    'No custom MSBuild targets found.'
  );
}
...
function packageLockPackagesTable(packages) {
  ...
  return actionTable(
    ['Target', 'Package', 'Type', 'Requested', 'Resolved', 'Content Hash', 'Dependencies'],
    rows,
    'No packages.lock.json entries found.'
  );
}
