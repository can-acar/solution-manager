...
exports.__test = exports.ProjectActions = void 0;
// @ts-nocheck
...
class ProjectActions {
    ...
    async addProjectReference(node) {
        ...
        this.terminalRunner.runCommand(`dotnet add ${(0, terminalRunner_1.quoteForShell)(project.path)} reference ${(0, terminalRunner_1.quoteForShell)(pick.item.path)}`);
    }
    ...
    async removeDependencyReference(node) {
        ...
        const answer = await vscode.window.showWarningMessage(`Remove ${node.label || include} from ${project.name}?`, { modal: true }, 'Remove');
        if (answer !== 'Remove') {
            return;
        }
        await (0, projectFileEditor_1.updateProjectItemReferences)(project.path, [{
    ...
    async removeNuGetPackage(node) {
        ...
        if (packageRefs.length > 0) {
            const pick = await vscode.window.showQuickPick(packageRefs.map((reference) => ({
    ...
    async runEfAction(action, node) {
        ...
        if (action === 'updateDatabase') {
            this.terminalRunner.runCommand(`dotnet ef database update --project ${(0, terminalRunner_1.quoteForShell)(project.path)}`);
            return;
        }
        this.terminalRunner.runCommand(`dotnet ef migrations script --project ${(0, terminalRunner_1.quoteForShell)(project.path)}`);
    }
    runDotnetAction(action, node) {
        const project = getProjectItem(node);
        this.terminalRunner.runCommand(`dotnet ${action} ${(0, terminalRunner_1.quoteForShell)(project.path)}`);
    }
    runGitAction(action, node) {
        const project = getProjectItem(node);
        const projectDirectory = path.dirname(project.path);
        if (action === 'status') {
            this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(projectDirectory)} status --short`);
            return;
        }
        if (action === 'diff') {
            this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(projectDirectory)} diff -- .`);
            return;
        }
        this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(projectDirectory)} log --oneline --decorate --max-count=30 -- .`);
    }
    ...
    async showProperties(node) {
        const project = getProjectItem(node);
        const metadata = await readProjectMetadata(project.path);
        metadata.launchSettings = await (0, launchSettingsEditor_1.readProjectLaunchSettings)(project.path);
        (0, projectPropertiesView_1.showProjectProperties)(this.context, project, metadata, async () => {
    ...
    async addConfiguration(project) {
        ...
        await (0, projectFileEditor_1.updateProjectProperties)(project.path, {
            configurations: [
                {
    ...
    async removeConfiguration(project, value) {
        ...
        const answer = await vscode.window.showWarningMessage(`Remove configuration "${parsed.configuration} | ${parsed.platform}" from ${project.name}?`, { modal: true }, 'Remove');
        if (answer !== 'Remove') {
            return;
        }
        await (0, projectFileEditor_1.removeProjectConfiguration)(project.path, parsed.configuration, parsed.platform);
        await this.refresh({ userVisible: true });
    }
    ...
    async promptProjectItemMetadata(elementName) {
        ...
        const outputPick = await vscode.window.showQuickPick(['Not specified', 'Never', 'PreserveNewest', 'Always'], {
            title: `${elementName}: CopyToOutputDirectory`
        });
        if (!outputPick) {
            return {};
        }
        const publishPick = await vscode.window.showQuickPick(['Not specified', 'Never', 'PreserveNewest', 'Always'], {
    ...
    async runProjectPropertiesAction(message, node) {
        ...
        if (action === 'restorePackages') {
            this.terminalRunner.runCommand(`dotnet restore ${(0, terminalRunner_1.quoteForShell)(project.path)}`);
        ...
        if (action === 'openProjectItem' || action === 'openProjectItemFolder' || action === 'copyProjectItemPath' || action === 'copyProjectItemXml' || action === 'removeProjectItem') {
            ...
            if (action === 'removeProjectItem') {
                const answer = await vscode.window.showWarningMessage(`Remove ${itemInfo.identity} from ${project.name}?`, { modal: true }, 'Remove');
                if (answer !== 'Remove') {
        ...
        if (action === 'publishWithProfile' || action === 'openPublishProfile' || action === 'copyPublishProfilePath' || action === 'copyPublishProfileXml') {
            ...
            if (action === 'publishWithProfile') {
                ...
                this.terminalRunner.runCommand(`dotnet publish ${(0, terminalRunner_1.quoteForShell)(project.path)} /p:PublishProfile=${(0, terminalRunner_1.quoteForShell)(profileName)}`);
        ...
        if (action === 'openProjectReference') {
            const metadata = await readProjectMetadata(project.path);
            const referenceInfo = findReference(metadata.projectReferences, value) || parseReferenceActionValue(value);
            const referencePath = await this.resolveProjectReferencePath(project, referenceInfo.path || referenceInfo.include || referenceInfo.name || value);
            if (!referencePath) {
                throw new Error('A project reference path is required.');
            }
        ...
        if (action === 'openLaunchSettings') {
            const launchPath = await (0, launchSettingsEditor_1.ensureLaunchSettingsFile)(project.path, project.name);
        ...
        if (action === 'addLaunchProfile') {
            ...
            await (0, launchSettingsEditor_1.addProjectLaunchProfile)(project.path, profileName.trim());
            return {
        ...
        if (action === 'removeLaunchProfile') {
            ...
            const answer = await vscode.window.showWarningMessage(`Remove run profile "${value}"?`, { modal: true }, 'Remove');
            if (answer !== 'Remove') {
                return { message: 'Remove run profile cancelled.' };
            }
            await (0, launchSettingsEditor_1.removeProjectLaunchProfile)(project.path, value);
            return {
        ...
        if (action === 'duplicateLaunchProfile') {
            ...
            await (0, launchSettingsEditor_1.duplicateProjectLaunchProfile)(project.path, value);
            return {
        ...
        if (action === 'browseLaunchExecutable' || action === 'browseLaunchWorkingDirectory') {
            ...
            await (0, launchSettingsEditor_1.updateProjectLaunchSettings)(project.path, [{
                    originalName: value,
                    name: value,
    ...
    async resolveProjectReferencePath(project, value) {
        ...
        const pick = await vscode.window.showQuickPick(references.map((reference) => ({
            label: reference.name,
        ...
        return resolveReferencePath(project.path, pick?.reference.path || pick?.reference.include || pick?.reference.name);
    }
}
...
async function readProjectMetadata(projectPath) {
    try {
        ...
        const metadata = (0, workspaceScanner_1.readProjectMetadataFromText)(Buffer.from(buffer).toString('utf8'), projectPath);
        metadata.centralPackageVersions = await (0, workspaceScanner_1.readCentralPackageVersions)(vscode.Uri.file(projectPath));
        metadata.packageReferences = (0, workspaceScanner_1.enrichPackageReferencesWithCentralVersions)(metadata.packageReferences, metadata.centralPackageVersions);
        metadata.sourceGenerators = enrichSourceGeneratorsWithPackageReferences(metadata);
        metadata.imports = await (0, workspaceScanner_1.enrichImportsWithImplicitDirectoryBuildFiles)(vscode.Uri.file(projectPath), metadata.imports);
        metadata.resolvedDependencies = await readResolvedDependencies(projectPath);
        metadata.nugetConfig = await (0, workspaceScanner_1.readNuGetConfig)(vscode.Uri.file(projectPath));
        metadata.packageReferences = (0, workspaceScanner_1.enrichPackagesWithPackageSourceMappings)(metadata.packageReferences, metadata.nugetConfig);
        metadata.resolvedDependencies = (0, workspaceScanner_1.enrichResolvedDependenciesWithPackageSourceMappings)(metadata.resolvedDependencies, metadata.nugetConfig);
        metadata.publishProfiles = await (0, workspaceScanner_1.readPublishProfiles)(vscode.Uri.file(projectPath));
        metadata.globalJson = await (0, workspaceScanner_1.readGlobalJson)(vscode.Uri.file(projectPath));
        metadata.packageLock = await (0, workspaceScanner_1.readPackageLockFile)(vscode.Uri.file(projectPath), metadata.buildSettings);
        metadata.userSecrets = await (0, workspaceScanner_1.readUserSecrets)(metadata.buildSettings?.userSecretsId);
        return metadata;
    }
    catch {
        return {
...
async function readResolvedDependencies(projectPath) {
    try {
        ...
        return (0, projectAssetsReader_1.readProjectAssetsFromText)(Buffer.from(buffer).toString('utf8'));
    }
    catch {
        return {};
    }
...
function parseConfigurationPropertyValue(value) {
    ...
    if (validateConfigurationPart(configuration)
...
function createProjectDirectoryCommand(projectPath, command) {
    ...
    if (process.platform === 'win32') {
        return `cd /d ${(0, terminalRunner_1.quoteForShell)(projectDirectory)} && ${command}`;
    }
    return `cd ${(0, terminalRunner_1.quoteForShell)(projectDirectory)} && ${command}`;
}
function createLaunchProfileRunCommand(project, profileName) {
    return `dotnet run --project ${(0, terminalRunner_1.quoteForShell)(project.path)} --launch-profile ${(0, terminalRunner_1.quoteForShell)(profileName)}`;
}
async function getLaunchProfileTerminalCwd(project, profileName) {
    const launchSettings = await (0, launchSettingsEditor_1.readProjectLaunchSettings)(project.path);
...
function getProjectPackageAssetGroups(packageInfo = {}) {
    ...
    return projectAssetsReader_1.PACKAGE_ASSET_GROUPS.map(([key, label]) => ({
        key,
        label,
