...
exports.SolutionActions = void 0;
// @ts-nocheck
...
class SolutionActions {
    ...
    async addSolutionFolder(solution) {
        ...
        const result = await (0, solutionFileEditor_1.addSolutionFolderToSolutionFile)(solution.path, folderPath.trim());
        if (!result.changed) {
    ...
    async addDockerComposeFile(solution) {
        ...
        vscode.window.setStatusBarMessage(exists ? 'Solution Manager: docker-compose.yml opened.' : 'Solution Manager: docker-compose.yml created.', 2500);
    }
    ...
    async addExistingProject(solution) {
        ...
        for (const uri of selection) {
            const result = await (0, solutionFileEditor_1.addProjectToSolutionFile)(solution.path, uri.fsPath, {
                solutionFolder
            });
    ...
    async addExistingFolder(solution) {
        ...
        const result = await (0, solutionFileEditor_1.addSolutionFolderToSolutionFile)(solution.path, relative);
        if (!result.changed) {
    ...
    async saveAs(solution) {
        ...
        await (0, solutionFileEditor_1.saveSolutionAs)(solution.path, targetUri.fsPath);
        await this.addCustomItems([targetUri]);
        await this.refresh({ userVisible: true });
        await this.openUri(targetUri);
    }
    runDotnetAction(solution, action) {
        this.terminalRunner.runCommand(`dotnet ${action} ${(0, terminalRunner_1.quoteForShell)(solution.path)}`);
    }
    async runMultipleProjects(solution) {
        ...
        for (const project of projects) {
            this.terminalRunner.runCommand(`dotnet run --project ${(0, terminalRunner_1.quoteForShell)(project.path)}`);
        }
    }
    runGitAction(solution, action) {
        const solutionDirectory = path.dirname(solution.path);
        if (action === 'status') {
            this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(solutionDirectory)} status --short`);
            return;
        }
        if (action === 'diff') {
            this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(solutionDirectory)} diff -- .`);
            return;
        }
        this.terminalRunner.runCommand(`git -C ${(0, terminalRunner_1.quoteForShell)(solutionDirectory)} log --oneline --decorate --max-count=30 -- .`);
    }
...
exports.SolutionActions = SolutionActions;
