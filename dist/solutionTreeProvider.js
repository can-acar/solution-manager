...
exports.__test = exports.SolutionTreeProvider = void 0;
// @ts-nocheck
...
class SolutionTreeProvider {
    ...
    async openSolutionFile() {
        ...
        const pick = await vscode.window.showQuickPick(candidates.map((item) => ({
            label: item.fileName,
    ...
    async runDependencyAction(action, node) {
        try {
            ...
            if (action === 'listPackages' && (node?.kind === 'dependencyGroup' || node?.kind === 'dependencies') && node.item?.path) {
                const command = `dotnet list ${(0, terminalRunner_1.quoteForShell)(node.item.path)} package --include-transitive`;
            ...
            if (action === 'updatePackage') {
                ...
                await (0, projectFileEditor_1.updateProjectItemReferences)(packageInfo.project.path, [{
                        action: 'add',
            ...
            if (action === 'removePackage') {
                const answer = await vscode.window.showWarningMessage(`Remove ${packageInfo.name} from ${packageInfo.project.name}?`, { modal: true }, 'Remove');
                if (answer !== 'Remove') {
                    return;
                }
                await (0, projectFileEditor_1.updateProjectItemReferences)(packageInfo.project.path, [{
            ...
            if (action === 'listPackages') {
                const command = `dotnet list ${(0, terminalRunner_1.quoteForShell)(packageInfo.project.path)} package --include-transitive`;
                this.terminalRunner.runCommand(command);
            }
        }
        catch (error) {
    ...
    async runDependencyReferenceAction(action, node) {
        try {
            ...
            if (action === 'removeProjectReference') {
                ...
                const answer = await vscode.window.showWarningMessage(`Remove ${referenceInfo.name} from ${referenceInfo.project.name}?`, { modal: true }, 'Remove');
                if (answer !== 'Remove') {
                    return;
                }
                await (0, projectFileEditor_1.updateProjectItemReferences)(referenceInfo.project.path, [{
                        action: 'remove',
    ...
    async runDependencyFrameworkAction(action, node) {
        try {
            ...
            if (action === 'build' || action === 'test') {
                const command = `dotnet ${action} ${(0, terminalRunner_1.quoteForShell)(node.item.path)} -f ${(0, terminalRunner_1.quoteForShell)(framework)}`;
                this.terminalRunner.runCommand(command);
            }
        }
        catch (error) {
...
exports.SolutionTreeProvider = SolutionTreeProvider;
...
function getDependencyImportChildren(project) {
    ...
    return imports.map((item) => createDependencyItem(project, 'import', item.name, item.source || sdk, item));
}
...
function createAssembliesGroup(project, framework, metadata, resolved) {
    ...
    const children = explicit.map((reference) => createDependencyItem(project, 'assemblies', getReferenceLabel(reference), getReferenceDescription(reference, 'assemblies'), reference));
    ...
    if (implicitAssemblies.length > 0) {
        children.push({
            ...
            item: project,
            children: implicitAssemblies.map((entry) => createDependencyItem(project, 'assembly', entry.name, entry.description, entry.reference)),
            collapsibleState: vscode.TreeItemCollapsibleState.Collapsed,
...
function getPackageDependencyGroups(project, framework, metadata, resolved) {
    if (!resolved || !resolved.packages || resolved.packages.length === 0) {
        return [
            createDependencyGroup(project, framework, 'packages', 'Packages', (0, workspaceScanner_1.enrichPackagesWithPackageSourceMappings)(metadata.packageReferences || [], metadata.nugetConfig))
        ];
    }
    const packages = (0, workspaceScanner_1.enrichPackagesWithPackageSourceMappings)(resolved.packages.map((item) => mergeResolvedPackageReference(item, metadata.packageReferences || [], framework)), metadata.nugetConfig);
...
function getFrameworkPackages(metadata, resolved, framework) {
    if (!resolved || !resolved.packages || resolved.packages.length === 0) {
        return (0, workspaceScanner_1.enrichPackagesWithPackageSourceMappings)(metadata.packageReferences || [], metadata.nugetConfig);
    }
    return (0, workspaceScanner_1.enrichPackagesWithPackageSourceMappings)(resolved.packages.map((item) => mergeResolvedPackageReference(item, metadata.packageReferences || [], framework)), metadata.nugetConfig);
}
...
function mergeResolvedProjectReference(resolvedProject, projectReferences, framework) {
    const projectReference = chooseProjectReferenceForFramework(projectReferences.filter((reference) => isMatchingProjectReference(resolvedProject, reference)), framework);
    if (!projectReference) {
...
function mergeResolvedFrameworkReference(resolvedFramework, frameworkReferences, framework) {
    const frameworkReference = chooseFrameworkReferenceForFramework(frameworkReferences.filter((reference) => {
...
function createDependencyGroup(project, framework, groupKind, label, references) {
    const children = references.map((reference) => createDependencyItem(project, groupKind, getReferenceLabel(reference), getReferenceDescription(reference, groupKind), reference));
    return {
...
function getDependencyPackageChildren(project, reference) {
    ...
    if (reference.dependencies && reference.dependencies.length) {
        children.push(createDependencyGroup(project, reference.key || reference.name, 'packageDependency', 'Dependencies', reference.dependencies));
    }
    for (const group of getPackageAssetGroups(reference)) {
        children.push(createDependencyGroup(project, reference.key || reference.name, getPackageAssetGroupKind(group.key), group.label, group.assets.map((asset) => ({
            name: asset,
            kind: group.key,
            assetPath: asset,
...
function hasDependencyPackageChildren(reference) {
    return Boolean(reference.dependencies?.length
...
function isPackageAssetGroupKind(groupKind) {
    return projectAssetsReader_1.PACKAGE_ASSET_GROUPS.some(([key]) => groupKind === getPackageAssetGroupKind(key));
}
function getPackageAssetGroupLabel(groupKind) {
    const group = projectAssetsReader_1.PACKAGE_ASSET_GROUPS.find(([key]) => groupKind === getPackageAssetGroupKind(key));
